...
Vagrant.configure('2') do |config|
  if (/darwin/ =~ RUBY_PLATFORM) != nil
      config.vm.box = "#{data['osx']['box']}"
      config.vm.box_url = "#{data['osx']['box_url']}"
  else
      config.vm.box = "#{data['linux']['box']}"
      config.vm.box_url = "#{data['linux']['box_url']}"
  end
